...
class Animation:
    def __init__(self, map_sizeX, map_sizeY, starts, goals, obstacles=[]):
        ...
        for o in self.obstacles:
            x, y = o[0], o[1]
            self.patches.append(
                Rectangle((x - 0.5, y - 0.5), 1, 1, facecolor='black'))
        ...
        for i in range(0, len(starts)):
            ...
            # self.T = max(self.T, len(starti[i])-1)
        ...
        self.init_func()
        # plt.savefig("weak_example.png",bbox_inches="tight",pad_inches=0.05)
    ...
    def generate_obs(self):
        for i in range(1, self.map_sizeX, 3):
            for j in range(1, self.map_sizeY, 3):
                self.patches.append(
                    Rectangle((i - 0.5, j - 0.5), 1, 1, facecolor='black'))
...
if __name__ == "__main__":
    ...
    map_sizeX, map_sizeY, obstacles = read_graph(args.map)
    starts, goals = load_env(args.schedule)
    map_sizeX, map_sizeY = map_sizeY, map_sizeX
    # paths,xmax,ymax,obstacles=load_env()

    # paths=load_schedule(args.schedule)
    # print(args.schedule)
    print("number of agents=", len(starts))
    animation = Animation(map_sizeX, map_sizeY, starts, goals, obstacles)
