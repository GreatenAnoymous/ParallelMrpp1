...
int Node::DiagonalDist(const Node &node) const{
    return std::max(std::abs(node.pos.x-this->pos.x),std::abs(node.pos.y-this->pos.y));
}

int Node::DiagonalDist(const Node *const node) const{
    return std::max(std::abs(node->pos.x-this->pos.x),std::abs(node->pos.y-this->pos.y));
}