...
public:
    ...
    {
        std::vector<std::vector<Node *>> connectedComponents;
        ...
        for (const auto &node : V)
        {
            ...
            {
                std::vector<Node *> connectedComponent;
                dfs(node, connectedComponent, visited);
    ...
    std::vector<Node *> getLargestConnectedComponent()
    {
        std::vector<std::vector<Node *>> connectedComponents = getConnectedComponents();

        if (connectedComponents.empty())
            return std::vector<Node *>(); // No connected components found

        auto maxComponent = std::max_element(
...
private:
    void dfs(Node *node, std::vector<Node *> &connectedComponent, std::vector<Node *> &visited)
    {
        visited.push_back(node);
        connectedComponent.push_back(node);

        for (const auto &neighbor : node->neighbor)
        {
            if (std::find(visited.begin(), visited.end(), neighbor) == visited.end())
            {
                dfs(neighbor, connectedComponent, visited);
            }
        }
    }
};
