...
const std::string ECBST::SOLVER_NAME = "ECBST";
const float ECBST::DEFAULT_SUB_OPTIMALITY = 2.0;

void dfs(int current, std::map<int, std::set<int>> &graph, std::map<int, bool> &visited, std::vector<int> &component)
{
...
std::vector<std::vector<int>> find_components(std::map<int, std::set<int>> &graph)
{
    ...
    for (const auto &vertex : graph)
    {
        if (!visited[vertex.first])
        {
            ...
            dfs(vertex.first, graph, visited, component);
...
ECBST::ECBST(Problem *_P) : Solver(_P)
{
    sub_optimality = DEFAULT_SUB_OPTIMALITY;
    solver_name = ECBST::SOLVER_NAME + "-" + std::to_string(sub_optimality);
}

ECBST::ECBST(Graph *graph, Config starts, Config goals) : Solver(graph, starts, goals)
{
    sub_optimality = DEFAULT_SUB_OPTIMALITY;
    solver_name = ECBST::SOLVER_NAME + "-" + std::to_string(sub_optimality);
}
...
void ECBST::generateChild(HighLevelNode_p &m, const HighLevelNode_p n, const LibCBS::Constraint_p c)
{
...
ECBST::CompareHighLevelNode ECBST::getMainObjective()
{
...
ECBST::CompareHighLevelNode ECBST::getFocalObjective()
{
    CompareHighLevelNode compare = [&](HighLevelNode_p a, HighLevelNode_p b)
    {
        if (a->f != b->f)
...
void ECBST::setInitialHighLevelNode(HighLevelNode_p n)
{
...
void ECBST::setInitialHighLevelNodeFocal(HighLevelNode_p n)
{
    n->paths = Paths();
    n->constraints.resize(P->getNum(), {});
...
Path ECBST::getInitialPath(int id, const Paths &paths)
{
...
void ECBST::invoke(HighLevelNode_p h_node, int id)
{
...
void ECBST::invokeDP(HighLevelNode_p h_node, int id)
{
...
void ECBST::countConflictsDP(HighLevelNode_p &h_node, int id)
{
    LibCBS::Conflicts cfs;
    LibCBS::getConflictDP(h_node->parent->paths, h_node->paths.get(id), id, cfs);
    for (auto &cf : h_node->parent->conflicts)
...
std::tuple<Path, int> ECBST::getFocalPath(HighLevelNode_p h_node, int id)
{
    Node *s = P->getStart(id);
    Node *g = P->getGoal(id);
...
Path ECBST::getInitialPathFocal(HighLevelNode_p h_node, int id)
{
    Node *s = P->getStart(id);
    Node *g = P->getGoal(id);
...
std::tuple<Path, int> ECBST::getTimedPathByFocalSearch(
...
Path ECBST::getPathFromFocalNode(FocalNode *_n)
{
    Path path;
    FocalNode *n = _n;
    while (n != nullptr)
...
void ECBST::setParams(int argc, char *argv[])
{
...
void ECBST::printHelp()
{
    std::cout << ECBST::SOLVER_NAME << "\n"
