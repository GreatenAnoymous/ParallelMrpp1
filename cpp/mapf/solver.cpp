...
Solver::Solver(Graph *G, Config starts, Config goals, int max_comp_time)
    : MinimumSolver(G, starts, goals, max_comp_time),
      verbose(false),
      LB_soc(0),
      LB_makespan(0),
      distance_table(P->getNum(),
...
Solver::~Solver()
{
    if (P->isInitializedInstance())
        LibCBS::MDD::PURE_MDD_TABLE.clear();
}
...
bool Solver::overCompTime() const
{
    ...
    if (getSolverElapsedTime() >= max_comp_time)
        std::cout << " over time,max comp time=" << max_comp_time << "elapsed time" << getSolverElapsedTime() << std::endl;
...
void Solver::computeLowerBounds()
{
    ...
    for (int i = 0; i < P->getNum(); ++i)
    {
        ...
        if (d == max_timestep)
        {
            d = pathDist(P->getStart(i), P->getGoal(i));
        }
        LB_soc += d;
        if (d > LB_makespan)
            LB_makespan = d;
...
void Solver::halt(const std::string &msg) const
{
    ...
    // this->~Solver();
    // std::exit(1);
    throw std::runtime_error("unexpected situation happened");
}
...
int Solver::pathDist(const int i, Node *const s) const
{
    ...
    // std::cout<<" is null "<<(s==nullptr)<<std::endl;
...
Path Solver::getPathBySpaceTimeAstar(Node *const s,
                                     Node *const g,
...
Path Solver::getPrioritizedPath(const int id,
                                const Paths &paths,
                                const int time_limit,
...
#pragma omp parallel for
    ...
    for (int i = 0; i < num_agents; ++i)
    {
        ...
        for (int t = 0; t <= makespan; ++t)
        {
            if (t >= PATH_TABLE.size())
                continue;

            PATH_TABLE[t][paths.paths[i][t]->id] = NIL;
        }
    }
}
...
void Solver::updatePathTableForAgent(const Path &p, int id)
{
    if (p.empty())
        return;

    const int makespan = (int)PATH_TABLE.size() - 1;
    const int nodes_size = G->getNodesSize();
    const int p_makespan = p.size() - 1;
    if (p_makespan > makespan)
    {
...
void Solver::clearPathTableForAgent(const Path &p, int id)
{
    if (p.empty())
        return;
    ...
    const int nodes_size = G->getNodesSize();
    const int p_makespan = p.size() - 1;
    assert(p_makespan <= makespan);

    for (int t = 0; t <= p_makespan; ++t)
    {
        if (PATH_TABLE[t][p[t]->id] == id)
        {
