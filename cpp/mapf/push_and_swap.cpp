...
void PushAndSwap::run()
{
    ...
    for (int i = 0; i < P->getNum(); ++i)
        occupied_now[solution.last(i)->id] = i;
                       ...
                       std::vector<int> &occupied_now)
{
    ...
    while (plan.last(id) != P->getGoal(id))
    {
        ...
        if (!pushTowardEmptyNode(v, plan, occupied_now, obs))
            return false;
    }
                       ...
                       std::vector<int> &occupied_now)
{
    ...
    while (!swap_verticies.empty() && !succcess)
    {
        ...
        {
            if (clear(tmp_plan, v, r, s, tmp_occupied_now))
                succcess = true;
        }
    }
    if (!succcess)
    ...
    // update occupancy
    for (int i = 0; i < P->getNum(); ++i)
        occupied_now[plan.last(i)->id] = NIL;
    // update plan
    plan += tmp_plan;
    // update occupancy
    for (int i = 0; i < P->getNum(); ++i)
        occupied_now[plan.last(i)->id] = i;
    ...
    if (inArray(P->getGoal(s), U))
        return resolve(plan, r, s, U, occupied_now);
    ...
    while (occupied_now[ideal_loc_s->id] != NIL)
    {
        ...
        if (occupied_now[p[1]->id] != NIL)
        {
            ...
            if (!pushTowardEmptyNode(p[1], plan, occupied_now, obs))
            {
                ...
                if (!swap(plan, _r, U, occupied_now))
                    return false;
            }
            else
            {
...
void PushAndSwap::checkConsistency(Plan &plan, std::vector<int> &occupied_now)
{
    ...
    for (int i = 0; i < P->getNum(); ++i)
    {
        if (occupied_now[c[i]->id] != i)
            halt("check consistency");
    }
}
                              ...
                              std::vector<int> &occupied_now)
{
    ...
    // error check
    if (empty2 == nullptr)
        halt("execute swap, failed to clear");
                                      ...
                                      const Nodes &obs)
{
    ...
    {
        if (occupied_now[p[i - 1]->id] == NIL)
            halt("node must be occupied");
...
void PushAndSwap::findNodesWithManyNeighbors()
{
    ...
    for (auto v : V)
        if (v->getDegree() >= 3)
            nodes_with_many_neighbors.push_back(v);
}
...
Plan PushAndSwap::compress(const Plan &plan)
{
    ...
    while (!sameConfig(new_plan.last(), P->getConfigGoal()))
    {
        ...
        for (int i = 0; i < P->getNum(); ++i)
        {
            ...
            while (t < makespan && v_current == plan.get(t + 1, i))
                ++t;
            internal_clocks[i] = t;
            // already reach its goal
            if (t == makespan)
            ...
            if (temp_orders[v_next->id].front() == i)
            { // move to v_next
