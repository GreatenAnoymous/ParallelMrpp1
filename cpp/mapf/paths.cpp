...
Paths::Paths(std::vector<Path> paths)
{
    this->paths = paths;
    makespan = getMaxLengthPaths();
}
...
bool Paths::empty(const int i) const
{
    const int paths_size = paths.size();
    if (!(0 <= i && i < paths_size))
        halt("invalid index, i=" + std::to_string(i) + "  num of agents=" + std::to_string(paths_size));
...
void Paths::insert(int i, const Path &path)
{
    ...
    if (!(0 <= i && i < paths_size))
        halt("invalid index");
    if (path.empty())
        halt("path must not be empty");
    const int old_len = paths[i].size();
    paths[i] = path;
    const int path_size = path.size();
    if (path_size - 1 == getMakespan())
        return;
    format(); // align each path size
    if (path_size < old_len)
    {
        shrink(); // cutoff additional configs
        format();
    }
...
void Paths::append(const Path &path)
{
    ...
    if (path_size - 1 > getMakespan())
    {
        makespan = path_size - 1;
        format(makespan);
    }
    else
    {

        while (paths[num_agents].size() - 1 < makespan)
        {
            paths[num_agents].push_back(path.back());
        }
    }
}
...
void Paths::format()
{
    ...
    for (int i = 0; i < paths_size; ++i)
    {
        ...
        while (p_size - 1 != len)
        {
            // std::cout<<paths[i].size()<<"  ????"<<std::endl;
...
bool Paths::conflicted(int i, int j, int t) const
{
    ...
    if (get(i, t) == get(j, t))
        return true;
    // swap conflict
    if (get(i, t) == get(j, t - 1) && get(j, t) == get(i, t - 1))
        return true;
    return false;
}
...
int Paths::countConflict(const std::vector<int> &sample) const
{
    ...
    for (int i = 0; i < sample_size; ++i)
    {
        for (int j = i + 1; j < sample_size; ++j)
        {
            for (int t = 1; t <= makespan; ++t)
            {
                if (conflicted(sample[i], sample[j], t))
                    ++cnt;
            }
        }
    }
...
int Paths::countConflict(int id, const Path &path) const
{
    ...
    for (int i = 0; i < num_agents; ++i)
    {
        ...
        for (int t = 1; t < path_size; ++t)
        {
            ...
            // swap conflict
            if (get(i, t) == path[t - 1] && get(i, t - 1) == path[t])
                ++cnt;
        }
    }
#else
    int cnt = 0;
    int makespan = paths[0].size() - 1;
...
#pragma omp parallel for reduction(+ : cnt) schedule(static)
    for (int i = 0; i < num_agents; ++i)
    {
        ...
        for (int t = 1; t < path_size; ++t)
        {
            if (t > makespan)
            {
                if (path[t] == paths[i][makespan])
                {
                    // Use atomic or reduction to update cnt
#pragma omp atomic
                    ++cnt;
                    break;
                }
...
#pragma omp atomic
            ...
            // swap conflict
            else if (paths[i][t] == path[t - 1] && paths[i][t - 1] == path[t])
            {
#pragma omp atomic
                ++cnt;
            }
