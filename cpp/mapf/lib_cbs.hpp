...
namespace LibCBS
{
    ...
    struct Conflict
    {
        ...
        Node *v = nullptr;
        Node *u = nullptr;
        Conflict(int _a1, int _a2, int _t, Node *_v, Node *_u)
            : t(_t), v(_v), u(_u), a1(_a1), a2(_a2){};
        Conflict(int _a1, int _a2, int _t, Node *_v)
            : a1(_a1), a2(_a2), t(_t), u(nullptr), v(_v){};
    };

    using Conflict_p = std::shared_ptr<Conflict>;
    using Conflicts = std::vector<Conflict_p>;

    void getAllConflicts(const Paths &paths, Conflicts &conflicts);
    void getAllConflictsDP(const Paths &paths, Conflicts &conflicts);
    void getAllConflictsParallel(const Paths &paths, Conflicts &conflicts, int numThreads);
    void getConflict(const Paths &paths, const Path &, int id, Conflicts &conflicts);
    void getConflictDP(const Paths &paths, const Path &, int id, Conflicts &conflicts);

    // for CBS-style solvers
    Constraints getFirstConstraints(const Paths &paths);
    Constraints getConstraintsFast(const Paths &paths);
    ...
    Constraints constraintsFromConflict(Conflict_p cf);

    Conflict_p chooseConflict(Conflicts &cfs);
    // ======================================
    // MDD
    struct MDDNode
    {
    ...
    struct MDD
    {
        ...
        Conflicts findAllConflicts(const Paths &paths);
        // Conflict findConflict(const Paths, )
