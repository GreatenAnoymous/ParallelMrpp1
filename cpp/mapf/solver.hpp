...
protected:
    ...
    int max_timestep;        // maximum makespan
    int max_comp_time;       // time limit for computation, ms
...
public:
    ...
    MinimumSolver(Graph *G, Config starts, Config goals, int max_comp_time = 60000)
    {
        this->G = G;

        P = new Problem(G, starts, goals, max_comp_time);

        max_timestep = P->getMaxTimestep();
        this->max_comp_time = max_comp_time;
        MT = P->getMT();
    }
    void setStarts(Config starts) { P->setStarts(starts); }
    void setGoals(Config goals) { P->setGoals(goals); }
...
public:
    virtual int getLowerBoundSOC();      // get trivial lower bound of sum-of-costs
    virtual int getLowerBoundMakespan(); // get trivial lower bound of makespan
private:
...
public:
     ...
     */
    static Path getPathBySpaceTimeAstar(Node *const s,                                // start
                                        Node *const g,                                // goal
...
protected:
    ...
    void updatePathTableForAgent(const Path &path,int id);
    void clearPathTable(const Paths &paths);
    void clearPathTableForAgent(const Path &path,int id);
...
public:
    Solver(Problem *_P);
    Solver(Graph *G, Config starts, Config goals, int max_comp_time = 130000);
    virtual ~Solver();
