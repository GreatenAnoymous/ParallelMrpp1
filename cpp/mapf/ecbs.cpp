...
const float ECBS::DEFAULT_SUB_OPTIMALITY = 1.5;
...
ECBS::ECBS(Graph *graph, Config starts, Config goals) : Solver(graph, starts, goals)
{
    sub_optimality = DEFAULT_SUB_OPTIMALITY;
    solver_name = ECBS::SOLVER_NAME + "-" + std::to_string(sub_optimality);
}
...
void ECBS::run()
{
    ...
    while (!OPEN.empty())
    {
        ...
        auto cf = LibCBS::chooseConflict(n->conflicts);
        LibCBS::Constraints constraints = LibCBS::constraintsFromConflict(cf);

        // if (n->parent != nullptr)
        // {
        //     std::cout << n->h_id << " parent id=" << n->parent->h_id<<"   cf=  " << cf->a1 << " " << cf->a2 << "  " << cf->t << "  (" << cf->v->pos.x << "," << cf->v->pos.y << ")==>";
        //     if (cf->u != nullptr)
        //         std::cout << cf->v->pos.x << "," << cf->v->pos.y << ")" << std::endl;
        //     else
        //         std::cout << std::endl;
        // }
...
void ECBS::setInitialHighLevelNodeFocal(HighLevelNode_p n)
{
    n->paths = Paths();
    ...
    LibCBS::getAllConflicts(n->paths, n->conflicts);
    n->f = n->conflicts.size();
    // assert(n->f == n->paths.countConflict());
...
void ECBS::invokeDP(HighLevelNode_p h_node, int id)
{
    auto res = getFocalPath(h_node, id);
    Path path = std::get<0>(res);
    int f_min = std::get<1>(res); // lower bound
    ...
    h_node->f = cfs.size();
    // auto nof_all = h_node->paths.countConflict();
    // auto nof_dp = h_node->f -
    //               h_node->paths.countConflict(id, h_node->paths.get(id)) +
    //               h_node->paths.countConflict(id, paths.get(id));
    // if (h_node->f != h_node->paths.countConflict())
    // {
    //     std::cout << h_node->f << "  " << nof_dp <<"   "<<nof_all<<std::endl;
    // }
    // assert(h_node->f == h_node->paths.countConflict());
...
Path ECBS::getInitialPathFocal(HighLevelNode_p h_node, int id)
{
    Node *s = P->getStart(id);
    Node *g = P->getGoal(id);
    ...
    CheckFocalFin &checkFocalFin, CheckInvalidFocalNode &checkInvalidFocalNode)
{
    ...
    {
        // std::cout<<G->getNodesSize() <<std::endl;
